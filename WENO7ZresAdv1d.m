function res = WENO7ZresAdv1d(w,flux,dflux,S,dx,N)
% *************************************************************************
...
% Constants
g0 = 1/35; g1 = 12/35; g2 = 18/35; g3 = 4/35; epsilon = 1e-40; tau7=abs(B0n-B3n);

% Alpha weights
alpha0n = g0.*(1 + tau7./(B0n + epsilon));
alpha1n = g1.*(1 + tau7./(B1n + epsilon));
alpha2n = g2.*(1 + tau7./(B2n + epsilon));
alpha3n = g3.*(1 + tau7./(B3n + epsilon));
...
% Constants
g0 = 4/35; g1 = 18/35; g2 = 12/35; g3 = 1/35; epsilon = 1e-40; tau7=abs(B0p-B3p);

% Alpha weights
alpha0p = g0.*(1 + tau7./(B0p + epsilon));
alpha1p = g1.*(1 + tau7./(B1p + epsilon));
alpha2p = g2.*(1 + tau7./(B2p + epsilon));
alpha3p = g3.*(1 + tau7./(B3p + epsilon));
...
res = (hp(:,I)+hn(:,I)-hp(:,I-1)-hn(:,I-1))/dx - S(w);